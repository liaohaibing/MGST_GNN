...
from torch.nn import Sequential as Seq, Linear as Lin, ReLU, LeakyReLU, ELU
...
class GlobalModel(torch.nn.Module):
    def __init__(self, aqi_em, rnn_h, rnn_l, gnn_h):
        ...
        self.aqi_em = aqi_em #输出aqi数据的特征维数16，通常就是embedding_dim(词向量的维度)
        self.rnn_h = rnn_h  #LSTM中隐层的维度64
        self.gnn_h = gnn_h  #32
        #self.aqi_embed = Seq(Lin(1, aqi_em), ReLU())#此处只考虑一个浓度值，可以尝试多个
        self.aqi_embed = Seq(Lin(1, aqi_em), ELU())  # 此处只考虑一个浓度值，可以尝试多个
        self.aqi_rnn = nn.LSTM(aqi_em, rnn_h, rnn_l, batch_first=True, bidirectional=False)  #循环神经网络的层数rnn_l=1
    ...
    def forward(self, city_aqi, city_conn, city_w, city_num):
        city_aqi = city_aqi.unsqueeze(dim=-1)#扩维
        city_aqi = self.aqi_embed(city_aqi)
        city_aqi, _ = self.aqi_rnn(city_aqi.reshape(-1, 24, self.aqi_em))
        #city_aqi = city_aqi.reshape(-1, 10, 24, self.rnn_h)#13个城市
        city_aqi = city_aqi.reshape(-1, city_num, 24, self.rnn_h)  #动态确定，这里是13个区
        city_aqi = city_aqi.transpose(1, 2)
        city_aqi = city_aqi.reshape(-1, city_num, city_aqi.shape[-1])#1536*13*64

        city_conn = city_conn.transpose(1, 2).repeat(24, 1, 1)#1536*2*156
        city_w = city_w.reshape(-1, city_w.shape[-2], city_w.shape[-1])#1536*156*2
...
class CityGNN(torch.nn.Module):
    def __init__(self, node_h, edge_h, gnn_h):#64，2，32
        super(CityGNN, self).__init__()
        self.node_mlp_1 = Seq(Lin(2 * node_h + edge_h, gnn_h),
                              ELU(inplace=True))
        self.node_mlp_2 = Seq(Lin(node_h + gnn_h, gnn_h), ELU(inplace=True))
    ...
    def forward(self, x, edge_index, edge_attr):
        ...
        # edge_attr: [E, F_e]
        row, col = edge_index
        out = torch.cat([x[row], x[col], edge_attr], dim=1)#239616*130
...
class CityModel(nn.Module):
    ...
    def __init__(self, aqi_em, poi_em, wea_em, rnn_h, rnn_l, gnn_h):
        ...
        self.gnn_h = gnn_h
        self.rnn_l = rnn_l
        self.aqi_embed = Seq(Lin(1, aqi_em), ELU())
        self.poi_embed = Seq(Lin(5, poi_em), ELU())
        self.city_embed = Seq(Lin(gnn_h, wea_em), ELU())
        self.wea_embed = Seq(Lin(10, wea_em), ELU())
        self.sta_gnn = StaGNN(aqi_em + poi_em, 2, gnn_h, 2 * wea_em)
        self.encoder = RNNEncoder(input_size=gnn_h,
                                  hidden_size=rnn_h,
                                  num_layers=rnn_l)
        self.decoder_embed = Seq(Lin(1, aqi_em), ELU())
        self.decoder = RNNDecoder(input_size=10 + aqi_em,
                                  hidden_size=rnn_h,
                                  num_layers=rnn_l)
